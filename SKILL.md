...
- Establish the main task, critical actions, content density, and target devices. Organize workspaces around objects and tools, and marketing pages around conversion. State reasonable assumptions and proceed; ask only about consequential missing information.
...
- Use [method selection](references/design-directions.md) to choose a suitable combination from the six core methods. Define each method's role, region, and shared visual rules, then read the selected recipes. An open brief requires making an appropriate choice, not bypassing the skill's methods. Respect local repairs and explicit style constraints without forcing effects into them.
...
- Reuse project components first. Implement missing mechanisms from selected recipes and verified cases, adapting them to the current brand, stack, and state. Case layouts, colors, and copy are not default templates.
...
Complete when the representative region has been inspected, the full page follows coherent rules, and the core task still works if visual enhancements fail.
